...
public class TemplateUtil {
	...
	private final static boolean templateCacheEnabled = EnvironmentConfiguration.isTemplateCacheEnabled();
	
	private final static XmlCompressor compressor = new XmlCompressor();
	
	/**
	 * Clears the cache
	 */
	public static synchronized void clearCache() {
		Logger.info("Template cache clear: Dumping " + TemplateUtil.templateCache.size() + " entries.");
		TemplateUtil.templateCache.clear();
	}
	...
	public static synchronized String getTemplate(final String filePath, final Map<String, String> replacements) {
		return TemplateUtil.getTemplate(filePath, replacements, "\\{", "\\}");
	}
	 ...
	 * @return
	 */
	public static synchronized String getTemplate(final String filePath, final Map<String, String> replacements, final String limiterLeft, final String limiterRight) {
		String content = "";
		try {
			...
			if (TemplateUtil.templateCacheEnabled && TemplateUtil.templateCache.containsKey(filePath)) {
				...
				// ** process include instruction: {include:filename}
				{
					...
					while (matcher.find()) {
						...
						includeMap.put(limiterLeft + "include:" + fileToInclude + limiterRight, TemplateUtil.getTemplate(currentFile.getParentFile().getAbsolutePath() + File.separator + fileToInclude, null));
					}
				}
				// **************
				...
				// ** process feature include instruction: {feature:FEATURE:filename}
				{
					...
					while (matcher.find()) {
						...
						if (FeatureToggle.isEnabled(Feature.valueOf(featureToCheck))) {
							includeMap.put(replacement, TemplateUtil.getTemplate(currentFile.getParentFile().getAbsolutePath() + File.separator + fileToInclude, null));
						} else {
				...
				content = content.replaceAll("\\{\\$BASEURL\\$\\}", EnvironmentConfiguration.getUrlBase());
				...
				if (TemplateUtil.templateCacheEnabled) {
					...
					TemplateUtil.templateCache.put(filePath, xmlcomp);
